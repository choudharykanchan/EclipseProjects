...
public class ProductDetailsPage extends RTPage {
	...
	@FindBy(css="div#backLink a")
	WebElement backLink;
	
	@FindBy(css="select.product-qty-select")
	WebElement qtySelect;
	
	@FindBy(css="div.popover-content")
	WebElement popover;
	
	@FindBy(css="div.putOnVehicle a")
	...
	public Product getProductDisplayed() {
		...
		String name = detailsContainer.findElement(By.cssSelector("h3.model")).getText();
		
		//brand sucks to grab from DOM. Just fair warning.
		//Example: <i class="rt-icon pirelli_small"></i>
		WebElement brandTag = detailsContainer.findElement(By.cssSelector("i.rt-icon"));
		...
		if (detailsContainer.findElements(By.className("rearQtySelect")).size() > 0) {
			...
			Price price = new Price(detailsContainer.findElement(By.cssSelector("h4.price-large")).getText());
